	...
	:	translation_unit	
		{
			...
			// static int start = 0;
			// static int translation_unit = 0;
			// outputTerminal(outP, "start_unit", start);
			// outP << " -> ";
			// outputTerminal(outP, "translation_unit", translation_unit);
			// start++;
			// translation_unit++;
	...
	:  declaration 
		{
			...
			$$->val = $1->val;
			$$->valType = $1->valType;
			$$->astPtr = new declList_Node($1->astPtr, NULL);
			registerNode(outA, $$->astPtr);
	 		outputNode(outA, $$->astPtr);
 			outA << " -> ";
 			outputNode(outA, $1->astPtr);
	...
	| declaration_list  declaration 
		{
			...
			$$->val = $1->val;
			$$->valType = $1->valType;
			$$->astPtr = new declList_Node($1->astPtr, $2->astPtr);
			registerNode(outA, $$->astPtr);
	 		outputNode(outA, $$->astPtr);
 			outA << " -> ";
 			outputNode(outA, $1->astPtr);
	...
	| declarator ASSIGN initializer
 		{ 
			...
			outG << "init_declarator -> {declarator ASSIGN initializer};" << std::endl; 
 			...
 			if (!$1->val._ste->setIdentifierValue(*$3)) {
			...
			$$->val = $1->val;
			$$->valType = $1->valType;
			$$->astPtr = new initDecl_Node($1->astPtr, $3->astPtr);
	 		registerNode(outA, $$->astPtr);
...
compound_statement
	...
	| LCURL open_curl statement_list RCURL close_curl
 		{
 			if(YFLAG){
...
postfix_expression
	...
	| postfix_expression LBRACK expression RBRACK /* COME BACK TO THIS */
 		{
 			if(YFLAG){
